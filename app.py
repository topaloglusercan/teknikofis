...
# --- MODÜLLER ---
def modul_ana_sayfa():
    st.title("🏗️ Teknik Ofis Portalı'na Hoş Geldiniz")
    st.markdown("""
    Bu portal, şantiye ve teknik ofis süreçlerinizi hızlandırmak için tasarlanmış ücretsiz araçlar sunar.
    
    👈 **Sol taraftaki menüden** kullanmak istediğiniz aracı seçebilirsiniz.
    
    **Mevcut Modüller:**
    * 📂 **İdari Hakediş Modülü:** Fiyat farkı hesabı, kova yönetimi ve ödenek teyit matrisi.
    
    *Not: Verileriniz sunucuda tutulmaz, %100 güvenlidir. Kaydettiğiniz projeler sadece sizin bilgisayarınızda barındırılır.*
    """)

def modul_idari_hakedis():
    ...
    with col1:
        ...
        edited_prog = st.data_editor(st.session_state.prog_df, num_rows="dynamic", use_container_width=True, key="ed_prog")
        st.subheader("3. Alt Endeks Ağırlıkları")
        edited_alt = st.data_editor(st.session_state.alt_df, num_rows="dynamic", use_container_width=True, key="ed_alt")
    with col2:
        st.subheader("2. Endeks Tablosu")
        edited_endeks = st.data_editor(st.session_state.endeks_df, num_rows="dynamic", use_container_width=True, key="ed_endeks")
        st.subheader("4. B Katsayısı Tablosu")
        edited_b = st.data_editor(st.session_state.b_df, num_rows="dynamic", use_container_width=True, key="ed_b")
...
def modul_yakinda():
    st.title("🚧 Yeni Modüller Yakında!")
    st.info("Bu alana ilerleyen günlerde Metraj, Yeşil Defter, Demir Hesabı gibi yeni teknik ofis araçları eklenecektir.")

# --- YAN MENÜ (SIDEBAR) YÖNETİMİ ---
st.sidebar.title("🛠️ Modül Seçimi")
secim = st.sidebar.radio(
    "Gitmek istediğiniz aracı seçin:",
    ["🏠 Ana Sayfa", "📂 İdari Hakediş Modülü", "🚧 Diğer Modüller (Yakında)"]
)

if secim == "🏠 Ana Sayfa":
    modul_ana_sayfa()
elif secim == "📂 İdari Hakediş Modülü":
    modul_idari_hakedis()
elif secim == "🚧 Diğer Modüller (Yakında)":
    modul_yakinda()

st.sidebar.markdown("---")
st.sidebar.caption("© 2026 Teknik Ofis Portalı")