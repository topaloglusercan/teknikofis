...
# ==========================================
def _format_decimal_max_6(val):
    """Türk/uluslararası sayı metnini en fazla 6 ondalıkla gösterir."""
    ...
    d = Decimal(s).quantize(Decimal('0.000001'), rounding=ROUND_HALF_UP)
...
def clean_df_for_ui(df, table_name=''):
    ...
    for col in df_clean.columns:
        ...
        is_numeric = col_upper not in ['AYLAR', 'AĞIRLIK', 'ENDEKS SÜTUNU']
        six_decimal_field = table_name == 'endeks' or col_upper == 'TEMEL ENDEKS'
        new_vals = []
        for val in df_clean[col]:
            ...
            if s.endswith(" 00:00:00"):
                ...
                try:
                    if six_decimal_field:
                        s = _format_decimal_max_6(s)
                    elif "," not in s:
                        fval = float(s)
                        if fval != 0 or "." in s:
...
def load_from_excel(file):
    ...
    for sheet in xls.sheet_names:
        if sheet in sheet_map:
            ...
            dfs[sheet_map[sheet]] = clean_df_for_ui(df, table_name=sheet)
    return dfs
...
if 'last_excel_signature' not in st.session_state: st.session_state.last_excel_signature = None
if 'last_json_signature' not in st.session_state: st.session_state.last_json_signature = None
...
if uploaded_excel is not None:
    ...
    excel_signature = hashlib.sha256(excel_bytes).hexdigest()
    if excel_signature != st.session_state.last_excel_signature:
        try:
            dfs = load_from_excel(io.BytesIO(excel_bytes))
            ...
            st.session_state.last_excel_signature = excel_signature
...
if uploaded_json is not None:
    ...
    json_signature = hashlib.sha256(json_bytes).hexdigest()
    if json_signature != st.session_state.last_json_signature:
        try:
            data = json.loads(json_bytes.decode('utf-8'))
            if 'prog' in data: st.session_state.prog_df = clean_df_for_ui(pd.DataFrame(data['prog']), table_name='IsProgrami')
            if 'endeks' in data: st.session_state.endeks_df = clean_df_for_ui(pd.DataFrame(data['endeks']), table_name='Endeks')
            if 'alt' in data: st.session_state.alt_df = clean_df_for_ui(pd.DataFrame(data['alt']), table_name='AltEndeks')
            if 'b' in data: st.session_state.b_df = clean_df_for_ui(pd.DataFrame(data['b']), table_name='B')
            st.session_state.last_json_signature = json_signature
            st.session_state.load_count += 1
            st.sidebar.success("✅ Proje yüklendi!")
        except Exception as e:
            st.sidebar.error(f"Hata: {e}")
else:
    st.session_state.last_json_signature = None
...
# Editörlerdeki güncel veriyi kalıcı state'e yaz.
st.session_state.prog_df = edited_prog.copy()
st.session_state.endeks_df = edited_endeks.copy()
st.session_state.alt_df = edited_alt.copy()
st.session_state.b_df = edited_b.copy()
