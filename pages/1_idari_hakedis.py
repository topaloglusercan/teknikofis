        ...
        for val in df_clean[col]:
            ...
            if s.endswith(" 00:00:00"):
                ...
                if is_numeric and "," not in s:
                    try:
                        # Ondalık kısmı budamadan koruma (6 hane vs)
...
with col1:
    ...
    # State'i dinamik kaydediyoruz
    st.session_state.prog_df = st.data_editor(st.session_state.prog_df, column_config=get_text_config(st.session_state.prog_df), num_rows="dynamic", use_container_width=True, key=f"prog_ed_{suffix}")
    st.subheader("3. Alt Endeks Ağırlıkları")
    st.session_state.alt_df = st.data_editor(st.session_state.alt_df, column_config=get_text_config(st.session_state.alt_df), num_rows="dynamic", use_container_width=True, key=f"alt_ed_{suffix}")

with col2:
    st.subheader("2. Endeks Tablosu")
    st.session_state.endeks_df = st.data_editor(st.session_state.endeks_df, column_config=get_text_config(st.session_state.endeks_df), num_rows="dynamic", use_container_width=True, key=f"end_ed_{suffix}")
    st.subheader("4. B Katsayısı Tablosu")
    st.session_state.b_df = st.data_editor(st.session_state.b_df, column_config=get_text_config(st.session_state.b_df), num_rows="dynamic", use_container_width=True, key=f"b_ed_{suffix}")
...
if st.button("🚀 Hesapla ve Sonuçları Göster", use_container_width=True, type="primary"):
    with st.spinner("Matematiksel motor çalışıyor..."):
        # Sonuçları session_state içine alıyoruz ki silinmesinler
...
if st.session_state.get('hesap_yapildi', False):
    ...
    if df_sonuc.empty:
        ...
        with tab4:
            try:
                df_prog_raw = filter_empty_rows(st.session_state.prog_df.copy())
                ...
                # NumPy array ile güvenli fill_between matrisi oluşturduk
